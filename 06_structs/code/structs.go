...
func describeGroup(g Group) string {
	...
	desc := fmt.Sprintf("The %s user group has %d users. Newest user:  %s, Accepting New Users:  %t",
...
func main() {
	...
	u3 := User{ID: 3, FirstName: "John", LastName: "Doe", Email: "john.doe@gmail.com"}

	g := Group{role: "admin", users: []User{u1, u2, u3}, newestUser: u2, spaceAvailable: true}
